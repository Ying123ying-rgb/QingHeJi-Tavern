...
try {
 ...
 await click('qhjt-tab-actors');await click('qhjt-add-character');await click('qhjt-add-user');
 ...
 await change('qhjt-active','actor_2');await click('qhjt-tab-state');await click('qhjt-plus');
 check(metadata.qingheji_tavern.actors.actor_2.currency.amount===1100,'Active actor currency');
 check(metadata.qingheji_tavern.actors.actor_1.currency.amount===1000,'Other actor untouched');
 check(get('qhjt-summary').textContent.includes('我'),'Summary uses protagonist');
